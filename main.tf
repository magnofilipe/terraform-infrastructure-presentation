...
resource "aws_security_group" "web" {
  ...
  description = "Security group para servidor web - permite SSH e HTTP"
  vpc_id      = aws_vpc.main.id
...
resource "aws_instance" "web" {
  ...
  # instala e inicia um servidor web simples
  user_data = <<-EOF
    #!/bin/bash
    yum update -y
    yum install -y httpd
    ...
    echo "<h1>Terraform Demo - Ambiente: ${var.environment}</h1>" > /var/www/html/index.html
    echo "<p>Instance Type: ${var.instance_type}</p>" >> /var/www/html/index.html
    echo "<p>Criado com Infraestrutura como Código!</p>" >> /var/www/html/index.html
  EOF
